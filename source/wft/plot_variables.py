...
class Visualization:
    ...
    def plot_time_series(variables: [pd.DataFrame, pd.Series]): # type: ignore
        ...
        if isinstance(variables,pd.DataFrame):
            ...
            fig,axes = plt.subplots(nrows = n_series, ncols = 1,figsize=(10, n_series*2))
    ...
    def plot_histogram(variables: [pd.DataFrame, pd.Series], name_var: Optional[ str ] = 'Variable_1'): # type: ignore
        ...
        if isinstance(variables,pd.DataFrame):
            ...
            for i, name in enumerate(variables.columns):
                ...
                plot_hist(variables[name], name, ax, fontsize=12)  
                axs.append(ax)   
            ...
            plot_hist(variables, 'Variable', plt, fontsize=12)
            plt.show()
    ...
    def plot_joint_distribution(
        ...
        plt.show()
    @staticmethod
    def plot_single_equity_cuve(returns: pd.Series, model:Callable,
                                    X:pd.DataFrame,long_stats: pd.Series, 
                                    short_stats: pd.Series, transfrom: str = None):
        ...
        if not long_stats.empty:
            ...
            first_train_index = thresh_longs.index[0]
        ...
        plt.title("Equity Curve", loc='center')
        plt.show()
